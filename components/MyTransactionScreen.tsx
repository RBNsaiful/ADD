...
const TransactionItem: FC<{ transaction: Transaction, texts: any, index: number, onDelete: (t: Transaction) => void }> = ({ transaction, texts, index, onDelete }) => {
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center space-x-3">
                    <div className="relative">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center p-1.5 overflow-hidden transition-colors ${hasLogo ? 'bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700' : 'bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800'}`}>
                             {hasLogo ? (
                                <img src={methodDetails.logo} alt={transaction.method} className="h-full w-full object-contain" />
                             ) : (
            ...
            {/* ONLY ALLOW HIDING FAILED TRANSACTIONS */}
...
const MyTransactionScreen: FC<MyTransactionScreenProps> = ({ user, texts, adCode, adActive }) => {
    ...
    // SOFT DELETE: Load hidden IDs from localStorage
    const [hiddenIds, setHiddenIds] = useState<Set<string>>(() => {
        const saved = localStorage.getItem(`hidden_txns_${user.uid}`);
        return saved ? new Set(JSON.parse(saved)) : new Set();
    });

    const isBn = texts.cancel !== 'Cancel';
    const depositLabel = isBn ? 'জমা' : 'DEPOSIT';
    ...
    const handleConfirmDelete = () => {
        if (transactionToDelete && user.uid && transactionToDelete.key) {
             ...
             // Wait for animation
             setTimeout(() => {
                // SOFT DELETE: Just add to hidden list, DO NOT remove from Firebase
                setHiddenIds(prev => {
                    const next = new Set(prev);
                    next.add(key);
                    localStorage.setItem(`hidden_txns_${user.uid}`, JSON.stringify(Array.from(next)));
                    return next;
                });
                setExitingIds(prev => {
                    const next = new Set(prev);
    ...
    // Filter out items that are marked as hidden by the user
    const visibleTransactionsList = transactions.filter(t => !hiddenIds.has(t.key || ''));

    const completedTransactions = visibleTransactionsList.filter(t => t.status === 'Completed');
    const totalDeposit = completedTransactions.reduce((acc, curr) => acc + curr.amount, 0);
    const completedCount = completedTransactions.length;

    const displayedTransactions = visibleTransactionsList.slice(0, displayLimit);
    const hasMore = visibleTransactionsList.length > displayLimit;
    ...
    return (
        <div className="p-4 animate-smart-fade-in pb-32">
             <div className="max-w-lg mx-auto">
                <div className="relative bg-gradient-to-br from-blue-700 via-indigo-800 to-slate-900 p-5 rounded-2xl text-white shadow-xl mb-5 overflow-hidden animate-smart-slide-down flex items-center justify-between">
                    <WavyPath />
                    <div className="relative z-10 flex flex-col justify-center">
                        ...
                        <p className="text-[8px] text-white/60 mt-1 font-semibold uppercase tracking-widest">{depositLabel} {completedCount}</p>
                    </div>
                ...
                <div className="space-y-3 mt-8">
                    {visibleTransactionsList.length > 0 ? (
                        <>
                            ...
                            {hasMore && (
                                <div className="mt-8 flex justify-center animate-smart-fade-in">
                                    <button 
